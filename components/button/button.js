...
const Button = ({
  ...
  const variantStyles = {
    primary: `bg-purple-primary hover:bg-purple-700 active:bg-purple-800 text-white 
              ${
                disabled
                  ? "opacity-60 cursor-not-allowed hover:bg-purple-primary"
                  : ""
              }`,
    ...
    secondary: `bg-blue-primary hover:bg-blue-700 active:bg-blue-800 text-white 
                ${
                  disabled
                    ? "opacity-60 cursor-not-allowed hover:bg-blue-primary"
                    : ""
                }`,
  ...
  const onChangeToggle = () => {
    const interval = setShowSoon(!showSoon);
    setTimeout(() => {
      setShowSoon(false);
    }, 2000);
  ...
  const content = (
    <>
      {leftIcon && (
        <span className={`inline-flex ${getIconSize(size)}`}>{leftIcon}</span>
      )}
      <span>{children}</span>
      {rightIcon && (
        <span
          className={`inline-flex ${getIconSize(
            size
  ...
  return (
    <motion.button
      onClick={soon ? onChangeToggle : onClick}
      className={`${buttonClasses} group cursor-pointer transition-all duration-300`}
      {...motionProps}
      disabled={disabled}
    >
      {showSoon ? <span className="flex items-center gap-1 "><Hourglass className="w-4 h-4 animate-spin"/> Coming Soon</span> : content}
    </motion.button>
  );
};
